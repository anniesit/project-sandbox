...
(function () {
  ...
  function mockFetch(root, st) {
    ...
    fetch(url, { credentials: "omit" })
      ...
      .catch(function (err) {
        console.error("[cooccur] mock load failed (" + url + "):", err);
      });
  }
  ...
  function buildModel(root, data) {
    ...
    var series = (Array.isArray(data.series) ? data.series : [])
      .map(function (s) {
        var counts = years.map(function (_, k) {
      ...
      .sort(function (a, b) {
        return b.total - a.total || (a.label < b.label ? -1 : 1);
      })
      .slice(0, TOP_N);
  ...
  function renderRows(root, st) {
    ...
    m.series.forEach(function (s, ki) {
      html +=
        '<button type="button" class="filmtv-cooccur-legend-item" data-key="' +
        esc(s.key) +
        '" aria-label="' +
        esc("加入搜尋：" + s.label + "（共" + fmt(s.total) + "篇）") +
        '">' +
        '<span class="filmtv-cooccur-legend-label">' +
        esc(s.label) +
        "</span>" +
        '<span class="filmtv-cooccur-legend-count">（共' +
        fmt(s.total) +
        "篇）</span>" +
        '<span class="filmtv-cooccur-legend-dot" style="background:' +
        s.color +
        '"></span>' +
        "</button>" +
        '<div class="filmtv-cooccur-strip" data-ki="' +
        ki +
        '"></div>';
    });
  ...
  function draw(root) {
    ...
    for (var ki = 0; ki < strips.length; ki++) {
      ...
      for (yi = 0; yi < ny; yi++) {
        s.push('<rect class="filmtv-cooccur-hit" data-ki="' + ki + '" data-yi="' + yi + '" x="' + round(PAD_X + yi * band) + '" y="0" width="' + Math.ceil(band) + '" height="' + H + '"/>');
      }
      s.push("</svg>");
    ...
    if (axis) {
      var aw = axis.clientWidth || W,
        ah = axis.clientHeight || AXIS_H;
      var apW = Math.max(1, aw - PAD_X * 2),
        aband = apW / ny;
      var step = labelStep(years, apW);
      var a = ['<svg class="filmtv-cooccur-svg-el" width="' + aw + '" height="' + ah + '" viewBox="0 0 ' + aw + " " + ah + '">'];
      for (var xi = 0; xi < ny; xi++) {
        if (years[xi] % step !== 0) continue;
        a.push('<text class="filmtv-cooccur-xlabel" x="' + round(PAD_X + (xi + 0.5) * aband) + '" y="15" text-anchor="middle">' + years[xi] + "</text>");
      }
      a.push("</svg>");
  ...
  function showTip(root, st, hit) {
    var ki = +hit.getAttribute("data-ki"),
      yi = +hit.getAttribute("data-yi");
    var m = st.model,
      row = m.series[ki];
    if (!row) return hideTip(st);
    var v = row.counts[yi] || 0,
      year = m.years[yi];

    st.tooltip.innerHTML =
      '<div class="filmtv-cooccur-tip-head">' +
      '<span class="filmtv-cooccur-tip-kw">' +
      esc(row.label) +
      "</span>" +
      '<span class="filmtv-cooccur-tip-year">' +
      year +
      " 年</span>" +
      "</div>" +
      '<div class="filmtv-cooccur-tip-count">' +
      fmt(v) +
      UNIT +
      "</div>";
  ...
  function fmt(n) {
    var x = Number(n);
    return isFinite(x) ? x.toLocaleString("en-US") : String(n);
  }
  function esc(str) {

...
(function () {
  ...
  function container() { return byId("js-page-container"); }
  function rootEl() { return scope.querySelector(".viewer-root") || (scope.querySelector ? scope : document.body); }
  function tpl(id) { var t = document.getElementById(id); return t && t.content ? t.content.cloneNode(true) : null; }
  function on(el, ev, fn) { if (el) el.addEventListener(ev, fn); }
  function setDisabled(el, v) { if (el) el.disabled = !!v; }
  function setText(el, txt) { if (el && el.textContent !== txt) el.textContent = txt; }
  function toggleClass(el, cls, v) { if (el) el.classList.toggle(cls, !!v); }
  ...
  function init(opts) {
    ...
    dataBaseUrl = opts.dataBaseUrl ||
      (scope.getAttribute && scope.getAttribute("data-src")) || DEFAULT_DATA_BASE;

    wireEvents();

    var url = readUrl();
    var bookNumber = opts.bookNumber || url.book ||
      (scope.getAttribute && scope.getAttribute("data-book-number"));
    if (!bookNumber) { console.error("[viewer] no book specified (URL ?book= or opts.bookNumber)"); return; }
  ...
  function load(bookNumber, nav) {
    ...
    return fetchBook(bookNumber)
      ...
      .catch(function (err) { console.error("[viewer] load failed:", err); });
  }
  ...
  function onPopState() {
    if (!state.book) return;
    var url = readUrl();
    var target = url.page ? parseInt(url.page, 10)
      : url.article ? (findArticle(state.book, url.article) || {}).pageStart
      : 1;
   ...
   * ============================================================ */
  function orientation() { return (state.book && state.book.bookOrientation) || "left"; }
  function isVerticalBinding() { var o = orientation(); return o === "top" || o === "bottom"; }
  // Which screen-arrow means "advance": right for left/top/bottom, left for right.
  function leftArrowIsNext() { return orientation() === "right"; }
  ...
  // Structure is re-cloned only when the layout OR (for single) scroll direction changes.
  function structureKey() {
    return state.layout + "|" + (state.layout === "single" ? state.scrollDirection : "flip");
  }
  function isFlipMode() {
    return state.layout !== "thumbnail" &&
      !(state.layout === "single" && state.scrollDirection !== "flip");
  }
  ...
  function renderLayout() {
    ...
    if (state.layout === "thumbnail") {
      var tg = tpl("tpl-layout-thumbnail"); if (tg) c.appendChild(tg);
      renderThumbnails();
      return;
    }
    ...
    if (state.layout === "single") {
      if (state.scrollDirection === "flip") {
        var s = tpl("tpl-layout-single"); if (s) c.appendChild(s);
    ...
    if (state.layout === "double") {
      var d = tpl("tpl-layout-double"); if (d) c.appendChild(d);
      markRotationTarget(c.querySelector(".page-spread") || c.firstElementChild);
      return;
    }
    if (state.layout === "ocr") {
      var o = tpl("tpl-layout-ocr"); if (o) c.appendChild(o);
      markRotationTarget(c.querySelector(".page-image"));
      return;
    }
  }
  ...
  function pageAt(idx) { return state.book.pages[idx - 1]; }
  ...
  function renderPageImage(slot, page) {
    if (!slot) return;
    if (!page) { slot.removeAttribute("src"); slot.alt = ""; slot.classList.add("is-empty"); return; }
    ...
    if (slot.tagName === "IMG") {
      ...
      slot.onerror = function () { slot.classList.add("is-error"); };
      slot.onload = function () { slot.classList.remove("is-error"); };
      return;
    }
    ...
    img.addEventListener("load", function () { slot.innerHTML = ""; slot.appendChild(img); markRotationTarget(img); });
    img.addEventListener("error", function () {
      var er = tpl("tpl-page-error");
      if (er) setText(er.querySelector(".page-error-label"), "無法載入：" + (page.label || page.file));
      slot.innerHTML = ""; if (er) slot.appendChild(er);
    });
    img.src = url;
  }
   ...
   * ============================================================ */
  function turnPage(direction) {                // direction: 'next' | 'previous'
    if (state.layout === "thumbnail") return;
    var total = state.book.pages.length;
    if (state.layout === "double") {
      ...
      if (direction === "next") { if (vis[vis.length - 1] >= total) return; setPage(spreadLead(vis[vis.length - 1] + 1, state.connectMode)); }
      else { if (vis[0] <= 1) return; setPage(spreadLead(vis[0] - 1, state.connectMode)); }
      return;
    }
  ...
  function onLeftArrow() { turnPage(leftArrowIsNext() ? "next" : "previous"); }
  function onRightArrow() { turnPage(leftArrowIsNext() ? "previous" : "next"); }
  ...
  function applyTransform() {
    ...
    t.style.transform =
      "translate(" + state.panX + "px," + state.panY + "px) rotate(" + state.rotation + "deg) scale(" + zoomScale() + ")";
  }
  function zoomScale() { var n = parseFloat(state.zoom); return isNaN(n) ? 1 : n / 100; }
  ...
  function clampPan(v, axis) {
    var t = getRotationTarget(); var c = container();
    if (!t || !c) return v;
  ...
  function toggleFullscreen() {
    ...
    if (!document.fullscreenElement) {
      if (el.requestFullscreen) el.requestFullscreen().catch(function (e) { console.error(e); });
  ...
  function setupScrollObserver() {
    ...
    scrollObserver = new IntersectionObserver(function (entries) {
      var best = entries.filter(function (e) { return e.isIntersecting; })
    ...
    container().querySelectorAll(".page-image[data-page-index]").forEach(function (img) { scrollObserver.observe(img); });
  }
  ...
  function renderThumbnails() {
    ...
    state.book.pages.forEach(function (page, i) {
      var item = tpl("tpl-thumbnail-item"); if (!item) return;
      ...
      if (img) { img.src = base + page.file; img.alt = page.label || ""; img.loading = "lazy"; img.draggable = false; applyAspect(img, page); }
  ...
  function highlightThumbnail() {
    var grid = byId("js-thumbnail-grid"); if (!grid) return;
  ...
  function renderOcrPanel() {
    ...
    if (!articles.length) { content.textContent = "無內文"; if (trigger) trigger.classList.add("is-hidden"); return; }
  ...
  function toggleOcrTocPopover() {
    ...
    if (!pop) { buildOcrTocPopover(); pop = byId("js-ocr-toc-popover"); }
    if (pop) pop.classList.toggle("is-open");
  }
  function buildOcrTocPopover() {
    var host = container().querySelector(".ocr-panel") || container();
    var frag = tpl("tpl-ocr-toc-popover"); if (!frag) return;
    host.appendChild(frag);
    refreshOcrTocList();
  }
  function refreshOcrTocList() {
    var list = byId("js-ocr-toc-list"); if (!list) return;
    var articles = articlesOnCurrentPage();
  ...
  function jumpToArticleInOcr(id) {
    ...
    var pop = byId("js-ocr-toc-popover"); if (pop) pop.classList.remove("is-open");
  }
  ...
  function updateToolbarControls() {
    ...
    var leftBtn = byId("js-prev-page"), rightBtn = byId("js-next-page");
    ...
    if (leftArrowIsNext()) {                     // right-bound book: left arrow advances
    ...
    // Connect-mode radios — only in double
    scope.querySelectorAll('input[name="connectMode"]').forEach(function (i) { i.disabled = state.layout !== "double"; });

    // Scroll-direction radios — V/H only in single; V only in thumbnail; flip only otherwise
    scope.querySelectorAll('input[name="scrollDirection"]').forEach(function (i) {
      if (isThumb) i.disabled = i.value !== "vertical";
      else if (state.layout === "single") i.disabled = false;
      else i.disabled = i.value !== "flip";
    });
  }
  ...
  function syncDropdown(id, value) {
    ...
    opts.forEach(function (o) {
      ...
      if (active) label = (o.textContent || "").trim();
    });
  ...
  function applyZoomClass() {
    var c = container(); if (!c) return;
    ZOOM_PRESETS.forEach(function (z) { c.classList.toggle("zoom-" + z, state.zoom === z); });
  ...
  function applyFullscreenClass() {
    var r = rootEl(); if (r) r.classList.toggle("is-fullscreen", state.isFullscreen);
  }
  ...
  function wireEvents() {
    ...
    on(pageInput, "keydown", function (e) { if (e.key === "Enter") { commitPageInput(); pageInput.blur(); } });
    ...
    on(layoutHidden, "input", function () { setLayout(layoutHidden.value); });
    var zoomHidden = scope.querySelector('#js-zoom-dropdown input[type="hidden"]');
    on(zoomHidden, "input", function () { setZoom(zoomHidden.value); });

    // scroll popover (custom)
    on(byId("js-scroll-popover-trigger"), "click", function (e) {
      e.stopPropagation();
      var pop = byId("js-scroll-popover"); if (pop) pop.classList.toggle("is-open");
    });
    scope.querySelectorAll('input[name="scrollDirection"]').forEach(function (i) {
      on(i, "change", function () { setScrollDirection(i.value); });
    });
    scope.querySelectorAll('input[name="connectMode"]').forEach(function (i) {
      on(i, "change", function () { setConnectMode(i.value); });
    });

    // fullscreen + rotation
    on(byId("js-fullscreen"), "click", toggleFullscreen);
    on(byId("js-rotate-cw"), "click", function () { rotate("cw"); });
    on(byId("js-rotate-ccw"), "click", function () { rotate("ccw"); });
    document.addEventListener("fullscreenchange", function () {
      state.isFullscreen = !!document.fullscreenElement; render();
    });
    ...
    on(c, "click", function (e) {
      ...
      if (ocrTrigger) { toggleOcrTocPopover(); return; }
      var ocrLi = e.target.closest && e.target.closest("#js-ocr-toc-list li");
      if (ocrLi) { jumpToArticleInOcr(ocrLi.getAttribute("data-article-id")); return; }
      var fUp = e.target.closest && e.target.closest("#js-ocr-font-increase");
      if (fUp) { adjustOcrFontSize("increase"); return; }
      var fDown = e.target.closest && e.target.closest("#js-ocr-font-decrease");
      if (fDown) { adjustOcrFontSize("decrease"); return; }
    });
    ...
    on(c, "dragstart", function (e) { e.preventDefault(); });
    on(c, "mousedown", function (e) { if (beginDrag(e.clientX, e.clientY)) e.preventDefault(); });
    document.addEventListener("mousemove", function (e) { moveDrag(e.clientX, e.clientY); });
    document.addEventListener("mouseup", endDrag);
    on(c, "touchstart", function (e) { if (e.touches.length === 1) beginDrag(e.touches[0].clientX, e.touches[0].clientY); }, { passive: true });
    document.addEventListener("touchmove", function (e) { if (dragStart && e.touches.length === 1) moveDrag(e.touches[0].clientX, e.touches[0].clientY); }, { passive: true });
    document.addEventListener("touchend", endDrag);
    window.addEventListener("resize", function () { if (state.book) render(); });
  ...
  function commitPageInput() {
    var input = byId("js-page-input"); if (!input) return;
    var n = parseInt(input.value, 10);
    if (isNaN(n)) { input.value = String(state.currentPage); return; }
    setPage(n);
  }
  ...
  function onKeydown(e) {
    ...
    switch (e.key) {
      case "ArrowLeft": onLeftArrow(); break;
      case "ArrowRight": onRightArrow(); break;
      case "PageUp": turnPage("previous"); break;
      case "PageDown": turnPage("next"); break;
      case "Escape":
        var s = byId("js-scroll-popover"); if (s) s.classList.remove("is-open");
        var o = byId("js-ocr-toc-popover"); if (o) o.classList.remove("is-open");
        break;
      default: return;
    }
  }
